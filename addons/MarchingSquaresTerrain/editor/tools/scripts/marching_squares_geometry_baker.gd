...
func bake_geometry_texture(inst: MeshInstance3D, scene_tree: SceneTree) -> void:
	...
	# Duplicate attribute arrays once for the new outset vertices (avoid appending an array to itself).
	var bake_uvs_copy := bake_uvs.duplicate()
	var bake_uv2s_copy := bake_uv2s.duplicate()
	var bake_normals_copy := bake_normals.duplicate()
	var bake_cols_copy := bake_cols.duplicate()
	var bake_c0_copy := bake_c0.duplicate()
	var bake_c1_copy := bake_c1.duplicate()
	var bake_c2_copy := bake_c2.duplicate()
	bake_uvs.append_array(bake_uvs_copy)
	bake_uv2s.append_array(bake_uv2s_copy)
	bake_normals.append_array(bake_normals_copy)
	bake_cols.append_array(bake_cols_copy)
	bake_c0.append_array(bake_c0_copy)
	bake_c1.append_array(bake_c1_copy)
	bake_c2.append_array(bake_c2_copy)
	var bake_c3 := _pack_verts_to_float_array(orig_verts)
	var bake_c3_copy := bake_c3.duplicate()
	bake_c3.append_array(bake_c3_copy)
